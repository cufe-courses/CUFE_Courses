...
\title{主标题}
\subtitle{副标题}
\institute[xx学院]{中央财经大学大学xx学院}%在`[]`中打入会在下栏中显示
\date{2025年5月20日}
\usepackage{cufe}
...
\begin{frame}
    \begin{itemize}
        ...
        \item 同时该模版还受到方匡南老师的启发
        \url{http://www.kuangnanfang.com/?id=8}
    \end{itemize}
\end{frame}
...
\begin{frame}{图形与分栏}
    % 在minipage环境中敲入`[position]`为分栏是否居中还是靠左对齐等，`[height]`为输入盒子高度，`{\linewidth}`输入盒子宽度
